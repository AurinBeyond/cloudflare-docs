...
This section only covers the inbound TLS connection from the end-user client to Cloudflare's edge. When a Worker fetches data from a backend storage service ([D1](/d1/), [KV](/kv/), [Durable Objects](/durable-objects/), [R2](/r2/), [Workers AI](/workers-ai/), [Hyperdrive](/hyperdrive/), and similar), that connection is governed by the [Cloudflare internal network](#cloudflare-internal-network) section. When a Worker calls out to a third-party origin via `fetch()`, it is governed by the [Cloudflare to origin](#cloudflare-to-origin) section. Cloudflare One products that have their own TLS termination at the edge (for example, [Agentless Cloudflare Access](#agentless-cloudflare-access) and [Cloudflare Gateway](#cloudflare-gateway)) are covered separately under [Cloudflare One](#cloudflare-one) below.
...
**Products covered:** [Workers VPC](/workers-vpc/) private-network access and any [Cloudflare One](/cloudflare-one/) off-ramp that egresses via `cloudflared` (for example, HTTPS access to self-hosted applications via [Agentless Cloudflare Access](#agentless-cloudflare-access)).
...
[Agentless Cloudflare Access](/cloudflare-one/access-controls/applications/) provides browser-only access to applications protected by Cloudflare Access. The browser establishes a TLS 1.3 connection to Cloudflare's edge, where Cloudflare Access enforces policy.
...
Reference: [PQC and Cloudflare One: Agentless Cloudflare Access](/ssl/post-quantum-cryptography/pqc-and-zero-trust/#agentless-cloudflare-access).

### Cloudflare Gateway

[Cloudflare Gateway](/cloudflare-one/traffic-policies/http-policies/tls-decryption/#post-quantum-support) is a Secure Web Gateway that runs on Cloudflare's edge and filters HTTPS traffic. The status below covers the inbound browser-to-Gateway TLS connection (including [agentless deployments via proxy endpoints / PAC files](/cloudflare-one/networks/resolvers-and-proxies/proxy-endpoints/)).
...
Gateway has no client-side component; clients can also reach Gateway via the [Cloudflare One Client](#cloudflare-one-client) or the [Cloudflare One Appliance](#cloudflare-one-appliance), which provide their own post-quantum on-ramps. The egress leg from Gateway to third-party origin servers is covered by [Cloudflare to origin](#cloudflare-to-origin) and is independent of the on-ramp.
